...
function Dashboard() {
  ...
  const handleDeleteSurvey = async (id: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this survey? All associated responses will be lost.',
      )
    ) {
      return
    }
    ...
    <div className="min-h-screen flex flex-col font-body-md bg-grid relative text-on-background">
      ...
      <nav className="bg-background w-full brutal-border-bottom border-on-background flex justify-between items-center px-margin-desktop h-16 max-w-container-max mx-auto z-40 bg-opacity-90 backdrop-blur-sm sticky top-0">
        ...
        <div className="hidden md:flex items-center gap-6 h-full">
          <Link
            to="/dashboard"
            className="h-full flex items-center text-primary underline underline-offset-4 decoration-3 font-label-lg text-label-lg px-4 brutal-border-l brutal-border-r border-transparent"
          >
            Dashboard
          </Link>
        </div>
        <div className="flex items-center gap-4">
          <button
            ...
            className="hidden md:flex items-center gap-2 bg-primary text-on-primary font-label-lg text-label-lg px-4 py-2 brutal-border hard-shadow hover:bg-surface-tint"
          >
          ...
          <div className="flex gap-2 items-center">
            <button
              onClick={() => logout()}
              title="Logout"
              className="p-2 text-on-surface hover:bg-secondary-container transition-colors duration-100 brutal-border border-transparent hover:border-on-background flex items-center justify-center"
            >
              <span className="material-symbols-outlined">logout</span>
            </button>
            <div
              className="w-10 h-10 brutal-border overflow-hidden bg-surface-container-high ml-2 relative group cursor-pointer"
              onClick={() => logout()}
              title="Logout"
            >
              <img
      ...
      <main className="flex-grow w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-8 flex flex-col gap-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b-3 border-on-background pb-4">
          <div>
            <div className="font-label-sm text-label-sm uppercase bg-secondary-container text-on-secondary-container px-2 py-1 inline-block mb-2 brutal-border">
              System Overview
            </div>
            ...
            <p className="font-label-lg text-label-lg text-outline">
              MANAGE YOUR DATA COLLECTION MODULES.
            </p>
          </div>
          <div className="flex gap-4">
            <div className="bg-surface brutal-border p-4 flex flex-col justify-between min-w-[140px] hard-shadow bg-secondary-container">
              ...
              <span className="font-headline-md text-headline-md font-bold mt-2">
            ...
            <div className="bg-surface brutal-border p-4 flex flex-col justify-between min-w-[140px] hard-shadow">
              ...
              <span className="font-headline-md text-headline-md font-bold mt-2 text-primary">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-surface-container-low p-4 brutal-border">
          ...
          <button
            onClick={handleCreateSurvey}
            className="hidden md:w-auto w-full md:flex items-center justify-center gap-2 bg-primary text-on-primary font-label-lg text-label-lg px-4 py-3 brutal-border hard-shadow active:translate-x-1 active:translate-y-1"
          >
            <span className="material-symbols-outlined text-[18px]">add</span>
            Create Survey
          </button>
        ...
        {filteredSurveys.length === 0 ? (
          <div className="text-center font-label-lg text-label-lg py-12 brutal-border bg-surface flex flex-col items-center gap-4 p-8">
            ...
            <button
              onClick={handleCreateSurvey}
              className="bg-primary text-on-primary font-label-lg text-label-lg px-6 py-3 brutal-border hard-shadow"
            >
              Create First Survey
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSurveys.map((survey) => (
              <div
                key={survey.id}
                className="bg-surface flex flex-col brutal-border relative group"
              >
                {/* Header Bar */}
                <div className="bg-on-background text-surface px-4 py-2 flex justify-between items-center border-b border-surface">
                  <span className="font-label-sm text-label-sm uppercase tracking-widest flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-none ${survey.status === 'published' ? 'bg-[#00ff00] animate-pulse' : 'bg-outline'}`}
                    ></span>
                    ID: {survey.slug}
                  </span>
                  <button
                    onClick={() => handleDeleteSurvey(survey.id)}
                    className="text-surface hover:text-error transition-colors"
                    title="Delete Survey"
                  >
                ...
                <div className="p-6 flex-grow flex flex-col gap-4">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-headline-sm text-headline-sm uppercase text-on-background line-clamp-1">
                        {survey.title}
                      </h3>
                      <span
                        className={`font-label-sm text-label-sm px-2 py-1 brutal-border border-[2px] uppercase ${
                  ...
                  <div className="grid grid-cols-2 gap-px bg-on-background mt-auto brutal-border border-[2px]">
                    <div className="bg-surface p-3 flex flex-col">
                      ...
                      <span className="font-label-lg text-label-lg text-on-background mt-1">
                    ...
                    <div className="bg-surface p-3 flex flex-col">
                      ...
                      <span className="font-label-sm text-label-sm text-on-background mt-2 flex items-center gap-1 font-bold">
                        <span
                          className="w-3 h-3 inline-block brutal-border border"
                ...
                <div className="border-t-2 border-on-background flex bg-surface-container-lowest">
                  <Link
                    ...
                    className="flex-1 py-3 font-label-sm text-label-sm uppercase hover:bg-secondary-container transition-colors border-r-2 border-on-background flex items-center justify-center gap-1"
                  >
                  ...
                  {survey.status === 'published' ? (
                    ...
                    <button
                      disabled
                      className="flex-1 py-3 font-label-sm text-label-sm uppercase opacity-40 border-r-2 border-on-background flex items-center justify-center gap-1 cursor-not-allowed"
                    >
                      <span className="material-symbols-outlined text-[16px]">share</span> Share
                    </button>
                  )}
                  <Link
                    ...
                    params={{ surveyId: survey.id }}
                    className="flex-1 py-3 font-label-sm text-label-sm uppercase bg-primary text-on-primary hover:bg-surface-tint transition-colors flex items-center justify-center gap-1"
                  >
            ...
            <header className="bg-on-background text-background flex justify-between items-center px-6 py-4 border-b-3 border-on-background">
              <div className="flex items-center gap-3">
                ...
                <h1 className="font-label-lg text-label-lg uppercase tracking-wider">
                  SHARE_MODULE // {activeShareSurvey.slug}
                </h1>
              </div>
              <button
                aria-label="Close modal"
                className="text-background hover:text-tertiary-fixed transition-colors"
            ...
            <div className="p-8 flex flex-col gap-8">
              ...
              <section className="flex flex-col gap-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <button
                    onClick={() =>
                      handleCopyLink(`${window.location.origin}/s/${activeShareSurvey.slug}`)
                    }
              ...
              <section className="flex flex-col gap-3">
                ...
                <div className="relative group">
                  ...
                  <button
                    onClick={() =>
                      handleCopyLink(
                        `<iframe src="${window.location.origin}/s/${activeShareSurvey.slug}" width="100%" height="600px" frameborder="0"></iframe>`,
                      )
                    }
