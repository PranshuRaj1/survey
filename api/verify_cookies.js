...
async function run() {
  ...
  console.log(
    'SUCCESS: Visitor A and Visitor B received distinct visitor_ids on KV cache-hit path.\n',
  )
  ...
  if (hasVisitorCookie) {
    console.error(
      'FAIL: Set-Cookie header returned even though client sent a valid visitor_id cookie!',
    )
    process.exit(1)
  }
  console.log(
    'SUCCESS: No Set-Cookie header returned when client already has a visitor_id cookie.\n',
  )
  ...
  if (analyticsData.visits !== 1) {
    console.error(
      `FAIL: Expected exactly 1 visit, but got ${analyticsData.visits}! Deduplication failed.`,
    )
    process.exit(1)
  }

  console.log(
    'SUCCESS: Cookie-based deduplication correctly counted exactly 1 visit from 2 sequential requests.',
  )
